...
\paragraph{Semantic separation from edge costs.}
A key design decision is the division of responsibilities between \emph{edge costs} and \emph{heuristic penalties}. Edge effective rates $r(e)$ already incorporate all \textbf{static fees}: taker/maker trading fees, withdrawal fees, and network gas fees. These are deterministic costs known at graph-construction time. In contrast, the heuristic penalties $h_i(n)$ capture \textbf{dynamic execution risk} factors---volume-based liquidity pressure, order-book depth, chain congestion, and exchange reliability---that are stochastic, time-varying, and depend on the trader's order size. This separation ensures there is no double-counting between $g(n)$ and $h(n)$.
...
The scores $s(n)$ are \textbf{static priors} assigned based on each exchange's operational track record, regulatory compliance status, and historical uptime. We set: Binance = 0.9, Kraken = 0.9, Coinbase = 0.9 (US-regulated, publicly traded), OKX = 0.8, KuCoin = 0.6, Bybit = 0.5, Bitget = 0.5, HTX = 0.5, Crypto.com = 0.6, Gate.io = 0.4, MEXC = 0.4, Phemex = 0.4. These scores reflect publicly available information about exchange withdrawal suspensions, regulatory actions, and community-reported incidents. Future work could replace these priors with rolling empirical measurements (e.g., API uptime over the past 30 days).
