...
\paragraph{Dijkstra remains a strong baseline.}
On a 41-node graph, Dijkstra discovers the highest-profit paths at moderate expansion cost. Its advantage is that it performs no heuristic computation, making it the natural choice when execution risk is not a concern.

\paragraph{3-hop enumeration is brittle.}
The structural baseline succeeds for only 30\% of start nodes and misses profitable 2-hop intra-exchange opportunities and 4+~hop cross-exchange paths that A*-based methods discover.
